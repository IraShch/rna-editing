...
fixed_chromosome_positions <- function(bam_file, current_chromosome, bam_index, pileup_params, fasta_file, 
                                       min_coverage, output_file, strand_specific, messy_file = NA, ambiguous_file = NA) {
  ...
  if (strand_specific) {
    ...
    bam_flag <- scanBamFlag(isPaired = TRUE, 
                            ...
                            hasUnmappedMate = FALSE)
    bam_params <- ScanBamParam(which = current_range, flag = bam_flag)
    pileup_table <- pileup(file = bam_file, 
                           index = bam_index, 
                           pileupParam = pileup_params, 
                           scanBamParam = bam_params)
...
# wrapper function
get_positions_from_bam <- function(bam_file_name, bam_index, reference_fasta, output_file, max_depth, min_base_quality, min_mapq, 
                                   min_nucleotide_depth, min_minor_allele_depth, min_coverage, use_noncanonical, strand_specific) {
  ...
  if (strand_specific) {
    n_path <- length(path)
    name_components <- unlist(strsplit(path[n_path], ".", fixed = TRUE))
    name_prefix <- str_c(name_components[1:length(name_components) - 1], collapse = ".")
    ...
    res <- lapply(chromosome_names, 
                  function(chr) fixed_chromosome_positions(bam_file, chr, bam_index, pileup_params, fasta_file, min_coverage, 
                                                           output_file, strand_specific, messy_file, ambiguous_file))
  } else {
    res <- lapply(chromosome_names, 
                  function(chr) fixed_chromosome_positions(bam_file, chr, bam_index, pileup_params, fasta_file, min_coverage, 
                                                           output_file, strand_specific))
  }
...
parser$add_argument('--nonStrandSpecific', help = 'non-strand-specific RNA-seq experiment', action = 'store_true')
...
strand_specific <- !args$nonStrandSpecific
...
# run 
get_positions_from_bam(bam_file, bam_index, reference_fasta, output_file, max_depth, min_base_quality, min_mapq, 
                       min_nucleotide_depth, min_minor_allele_depth, min_coverage, use_noncanonical, strand_specific)

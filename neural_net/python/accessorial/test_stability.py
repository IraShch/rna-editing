...
def create_model(X_train, y_train, nodes_number, batch_size, nb_epoch, include_coverage, loss, opt, scale_in_groups,
    ...
    if include_coverage and scale_in_groups == 2:
        model.add(Dense(nodes_number, input_dim=6, init='uniform', activation='tanh', W_regularizer=l2(.01)))
    elif include_coverage and scale_in_groups != 2:
        model.add(Dense(nodes_number, input_dim=5, init='uniform', activation='tanh', W_regularizer=l2(.01)))
    else:
        model.add(Dense(nodes_number, input_dim=4, init='uniform', activation='tanh', W_regularizer=l2(.01)))
    model.add(Dense(4, init='uniform', activation=activation, W_regularizer=l2(.01)))
